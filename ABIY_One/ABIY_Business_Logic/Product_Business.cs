...
namespace ABIY_One.ABIY_Business_Logic
{
    public class Product_Business
    {
        ...
        {
            return db.Products.Include(i => i.Category).ToList();//.Include();
        }
        public bool add(Product model)
        {
            try
            {
                var item = db.Products.Where(x => x.Category_ID == model.Category_ID && x.Description == model.Description && x.Name == model.Name && x.Promotion==model.Promotion).FirstOrDefault();
                if (item != null)
                {
